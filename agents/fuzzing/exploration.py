...
prompt = '''You are a website exploration agent focused on mapping interactive elements. For (maximum 5) EACH INTERNAL PAGE (same domain only):
...
Never analyze external domains. Focus on element discovery, not vulnerability classification. Once the plan is approved by a human, transfer the context and data to appropriate agent mentioned by the human.'''
...
new_tools = browser_tools + [make_handoff_tool(agent_name="supervisor")]
...
async def exploration_node(state: MessagesState) -> Command[Literal["supervisor", "human"]]:
